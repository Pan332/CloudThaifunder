...
export const GoogleAuth = async function (req, res, next) {
    res.header('Access-Control-Allow-Origin', 'http://54.204.61.104:80');
    res.header('Referrer-Policy', 'no-referrer-when-downgrade');

    const redirecturl = `http://54.204.61.104:${port}/Oauth2/google/callback/`;
...
export const GoogleCallback = async function (req, res, next) {
    ...
    try {
        ...
        if (users){
            ...
            res.redirect(`http://54.204.61.104:80?token1=${accessToken}&token2=${refreshToken}`);
        } else {
